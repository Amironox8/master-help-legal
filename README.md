...
В `FORM_ENDPOINT` стоит алиас FormSubmit, поэтому адрес получателя в коде страницы не виден.
Запасной контакт для случая сбоя отправки собирается в JS из частей — по той же причине.

Активация адреса в FormSubmit выполняется один раз, по ссылке из письма сервиса.
Если письма перестанут доходить — проверять в первую очередь её и лимиты сервиса.
